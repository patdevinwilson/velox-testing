...
# Install GCC 13 for GLIBCXX_3.4.31 support (required by conda cuDF 25.12)
RUN dnf install -y gcc-toolset-13 --setopt=install_weak_deps=False || true
...
RUN /usr/local/bin/micromamba create -y -n ucxx -c rapidsai -c conda-forge \
     ucx-proc=*=gpu ucx libucxx libcudf cuda-version=12.9 \
...
ARG NUM_THREADS=12
ARG EXTRA_CMAKE_FLAGS="-DPRESTO_ENABLE_TESTING=OFF -DPRESTO_ENABLE_PARQUET=ON -DPRESTO_ENABLE_CUDF=${GPU} -DVELOX_BUILD_TESTING=OFF -DVELOX_USE_FETCHCONTENT_UCXX=ON -DVELOX_USE_FETCHCONTENT_CUDF=OFF -DTHRUST_IGNORE_CUB_VERSION_CHECK=ON"
...
RUN --mount=type=bind,source=presto/presto-native-execution,target=/presto_native_staging/presto,rw \
    ...
    LD_LIBRARY_PATH=${UCXX_ENV_DIR}/lib:/opt/rh/gcc-toolset-13/root/usr/lib64:/usr/local/lib \
    LIBRARY_PATH=${UCXX_ENV_DIR}/lib:/opt/rh/gcc-toolset-13/root/usr/lib64:/usr/local/lib \
    CMAKE_PREFIX_PATH=${UCXX_ENV_DIR} \
    ucx_DIR=${UCXX_ENV_DIR}/lib/cmake/ucx \
    UCX_DIR=${UCXX_ENV_DIR}/lib/cmake/ucx \
    cudf_DIR=${UCXX_ENV_DIR}/lib/cmake/cudf \
    rmm_DIR=${UCXX_ENV_DIR}/lib/cmake/rmm \
    CXX=/opt/rh/gcc-toolset-13/root/usr/bin/g++ \
    CC=/opt/rh/gcc-toolset-13/root/usr/bin/gcc \
    LDFLAGS="-L${UCXX_ENV_DIR}/lib -Wl,-rpath,${UCXX_ENV_DIR}/lib -lstdc++" \
    CONDA_PREFIX= \
    ...
    !(LD_LIBRARY_PATH=${UCXX_ENV_DIR}/lib:/opt/rh/gcc-toolset-13/root/usr/lib64:/usr/local/lib:/usr/local/lib64 ldd ${BUILD_BASE_DIR}/presto_cpp/main/presto_server | grep -v "libcuda\.so\.1" | grep "not found") && \
    LD_LIBRARY_PATH=${UCXX_ENV_DIR}/lib:/opt/rh/gcc-toolset-13/root/usr/lib64:/usr/local/lib:/usr/local/lib64 ldd ${BUILD_BASE_DIR}/presto_cpp/main/presto_server | grep -v "libcuda\.so\.1" | awk 'NF == 4 { system("cp " $3 " /runtime-libraries") }' && \
...
RUN mkdir /usr/lib64/presto-native-libs && \
    ...
    rm -f /usr/lib64/presto-native-libs/libucxx.so* /usr/lib64/presto-native-libs/librmm.so* /usr/lib64/presto-native-libs/libcudf.so* && \
    echo "/usr/lib64/presto-native-libs" > /etc/ld.so.conf.d/presto_native.conf && \
    ldconfig && \
    echo "✓ Copied runtime libraries (using conda cuDF/RMM/UCXX 25.12 for compatibility)"
